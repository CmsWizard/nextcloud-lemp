...
echo "================================================================"
echo "======== A robot is now installing Nextcloud for you ======="
echo "========================== ETC = 120s =========================="
echo "================================================================"
...
if $varnaked;
  then sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/nextcloud-lemp/main/config.txt' -O /etc/nginx/sites-enabled/nextcloud && sudo sed -i "s/domain/$var01/g" /etc/nginx/sites-enabled/nextcloud;
elif $varwww;
  then sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/nextcloud-lemp/main/config-www.txt' -O /etc/nginx/sites-enabled/nextcloud && sudo sed -i "s/domain/$var011/g" /etc/nginx/sites-enabled/nextcloud;
else 
  sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/nextcloud-lemp/main/config-non-www.txt' -O /etc/nginx/sites-enabled/nextcloud && sudo sed -i "s/domain/$var01/g" /etc/nginx/sites-enabled/nextcloud;
fi
...
# installing the app 
cd /tmp
sudo wget https://download.nextcloud.com/server/releases/nextcloud-22.2.0.tar.bz2 -O latest.tar.bz2
tar -xf latest.tar.bz2
#sudo mv nextcloud "$var01"
#cd
...
sudo chmod -R 0755 /var/www/nextcloud
...
echo "======================= DONE! ========================"
echo "========= Nextcloud is installed sucessfully ========="
echo "================ =================== ================="
